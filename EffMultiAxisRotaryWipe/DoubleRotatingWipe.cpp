...
CDoubleRotatingWipe::CDoubleRotatingWipe(void)
{
	m_pMesh = NULL;
}
...
HRESULT CDoubleRotatingWipe::Init(CRenderEngine* pEngine)
{
	...
	m_pEffect = pResMan->CreateEffect(pDevice, _T("NewEffects/GMARW_DoubleRotating_Mask.fx"));
	ASSERT(m_pEffect);

	return hr;
}

void CDoubleRotatingWipe::Ready( CVideoBuffer* pMaskDef )
{
	CWipeBase::Ready(pMaskDef);
}

HRESULT CDoubleRotatingWipe::Draw(CVideoBuffer* pMaskDef, BasicWipeFxParam* pParam)
{
	HRESULT hr = E_FAIL;
	...
	D3DXMATRIXA16 matPrevTrans,matAxisTrans,matRot,matAxisInvTrans,matTrans,matMirror,
		matWorld,matCombine,matMirrorCombine,matType,matShift,matAdd;
...
HRESULT CDoubleRotatingWipe::InitMesh(CRenderEngine* pEngine) 
{
	HRESULT hr = E_FAIL;
	...
	if(!m_pMesh)
	{
		...
		pVertices[0] = D3DXVECTOR3(-10.0f,0.5f,0.0f);
		pVertices[1] = D3DXVECTOR3(0.0f,10.0f,0.0f);
		pVertices[2] = D3DXVECTOR3(10.5f,0.5f,0.0f);	

		pVertices[3] = D3DXVECTOR3(-10.0f,0.5f,1.0f);		
		pVertices[4] = D3DXVECTOR3(10.0f,0.5f,1.0f);	
