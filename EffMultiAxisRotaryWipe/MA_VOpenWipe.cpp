...
CMA_VOpenWipe::CMA_VOpenWipe(void)
{
	m_pMesh = NULL;
}
...
HRESULT CMA_VOpenWipe::Init(CRenderEngine* pEngine)
{
	...
	m_pEffect = pResMan->CreateEffect(pDevice, _T("NewEffects/GMARW_VOpen_Mask.fx"));
	ASSERT(m_pEffect);

	return hr;
}

void CMA_VOpenWipe::Ready( CVideoBuffer* pMaskDef )
{
	CWipeBase::Ready(pMaskDef);
}

HRESULT CMA_VOpenWipe::Draw(CVideoBuffer* pMaskDef, BasicWipeFxParam* pParam)
{
	HRESULT hr = E_FAIL;
	...
	D3DXMATRIXA16 matPrevTrans,matAxisTrans,matRot,matAxisInvTrans,matTrans,matMirror,matWorld,matType,matCombine[4],matShift,matAdd[4];
	...
	D3DXMatrixIdentity(&matAdd[0]);
	D3DXMatrixIdentity(&matAdd[1]);
	D3DXMatrixIdentity(&matAdd[2]);
	D3DXMatrixIdentity(&matAdd[3]);
	...
	case 12 :
		...
		matAdd[1]._41 = -1.0f  / biMask.nWidth;
		matAdd[3]._41 = -1.0f  / biMask.nWidth;
		break;
	}
	...
	D3DXMATRIXA16 matTemp = matPrevTrans * matType * matShift * matAxisTrans * matRot * matAxisInvTrans;
	matWorld = matTemp * matMirror * matTrans;
	matCombine[0] = matWorld * m_matView * m_matProj;

	matMirror._11 *= -1;	
	matWorld = matTemp * matMirror * matTrans  * matAdd[1];
	matCombine[1] = matWorld * m_matView * m_matProj;

	matMirror._22 *= -1;
	matWorld = matTemp * matMirror * matTrans * matAdd[3];
	matCombine[3] = matWorld * m_matView * m_matProj;

	matMirror._11 *= -1;	
	matWorld = matTemp * matMirror * matTrans * matAdd[2];
...
HRESULT CMA_VOpenWipe::InitMesh(CRenderEngine* pEngine) 
{
	HRESULT hr = E_FAIL;
	LPCTSTR pszMeshName = _T("MA_VOpenWipeMesh");
	LPDIRECT3DDEVICE9 pDevice = m_pEngine->GetDevice();
	...
	if(!m_pMesh)
	{
		...
		pVertices[0] = D3DXVECTOR3(0.0f,-9.5f,0.0f);
		pVertices[1] = D3DXVECTOR3(-10.0f,0.5f,0.0f);
		pVertices[2] = D3DXVECTOR3(0.0f,10.5f,0.0f);	

		pVertices[3] = D3DXVECTOR3(0.0f,-9.5f,1.0f);		
		pVertices[4] = D3DXVECTOR3(0.0f,10.5f,1.0f);	
