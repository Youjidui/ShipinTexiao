...
CCenterVOpenWipe::CCenterVOpenWipe(void)
{
	m_pMesh = NULL;
}
...
HRESULT CCenterVOpenWipe::Init(CRenderEngine* pEngine)
{
	...
	m_pEffect = pResMan->CreateEffect(pDevice, _T("NewEffects/GMARW_CenterVOpen_Mask.fx"));
	ASSERT(m_pEffect);

	return hr;
}

void CCenterVOpenWipe::Ready( CVideoBuffer* pMaskDef )
{
	CWipeBase::Ready(pMaskDef);
}

HRESULT CCenterVOpenWipe::Draw(CVideoBuffer* pMaskDef, BasicWipeFxParam* pParam)
{
	HRESULT hr = E_FAIL;
	...
	D3DXMATRIXA16 matPrevTrans,matAxisTrans,matRot,matAxisInvTrans,matTrans,matMirror,
		matWorld,matType,matCombine[4],matShift;
	...
	{
	case 13:
		vCenter = D3DXVECTOR2(-0.25f,0.-0.0f);
...
HRESULT CCenterVOpenWipe::InitMesh(CRenderEngine* pEngine) 
{
	...
	if(!m_pMesh)
	{
		...
		pVertices[0] = D3DXVECTOR3(-10.25f,0.0f,0.0f);
		pVertices[1] = D3DXVECTOR3(-0.25f,10.0f,0.0f);
		pVertices[2] = D3DXVECTOR3(9.75f,0.0f,0.0f);	

		pVertices[3] = D3DXVECTOR3(-10.25f,0.0f,1.0f);		
		pVertices[4] = D3DXVECTOR3(9.75f,0.0f,1.0f);	
